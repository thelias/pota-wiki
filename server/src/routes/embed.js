...
const LOGO_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="60 265 195 190" height="28" style="flex-shrink:0">
...
function renderStats(stats, callsign, baseUrl) {
  return `
    <div class="stats-panel">
      ...
      <div class="stat-row">
        <span class="stat-num">${stats.park_count ?? 0}</span>
        <span class="stat-desc">unique park${stats.park_count !== 1 ? 's' : ''}</span>
      </div>
      <div class="stat-row">
        <span class="stat-num">${stats.helpful_count ?? 0}</span>
        <span class="stat-desc">helpful vote${stats.helpful_count !== 1 ? 's' : ''}</span>
      </div>
...
function renderReport(r, baseUrl) {
...
function renderEmpty(baseUrl, callsign) {
  return `
    <div class="report-panel" style="display:flex;align-items:center;justify-content:center;color:${TEXT_MUTED};font-size:0.85rem;">
      No activation reports found for ${callsign}.
    </div>`
}
...
<head>
  ...
  <title>POTA Wiki — ${callsign}</title>
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .field-label { color: ${TEXT_MUTED}; font-weight: 500; }
    ...
    const [{ rows: reportRows }, { rows: statsRows }] = await Promise.all([
      ...
      pool.query(
        `SELECT
           u.report_count,
           u.helpful_count,
           COUNT(DISTINCT ar.park_reference)::int AS park_count
         FROM users u
         LEFT JOIN activation_reports ar ON UPPER(ar.callsign) = $1
         WHERE UPPER(u.callsign) = $1
